...
<svelte:head>
	...
	<meta name="description" content={i18n.t('signIn.description')} />
</svelte:head>
	...
	<div class="w-full max-w-md">
		...
		<article class="card-base p-8 md:p-10 space-y-6 shadow-sm">
			...
			<div class="text-center space-y-2">
				<h1 class="text-primary text-2xl md:text-3xl font-bold">{i18n.t('signIn.header')}</h1>
			</div>
			...
			<!-- Sign up & Reset password Link -->
			<div class="text-center">
				<p class="text-muted-foreground">
					{i18n.t('resetPassword.suggestion')}
					<button
						onclick={() => goto('/sign-in')}
						class="text-primary font-medium hover:text-primary/90 transition-colors"
					>
						{i18n.t('resetPassword.action')}
					</button>
				</p>
				<p class="text-muted-foreground">
					{i18n.t('signUp.suggestion')}
					<button
